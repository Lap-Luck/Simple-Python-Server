...
def play(player_id,sever_talker):
    ...
    playing=True
    move_num=0
    if full_print:print("Enter game",player_id)
    while playing:
        while True:
            can_move=sever_talker('{"type":"can_move","id":"' + str(player_id) + '"}')=='True'
            if full_print: print("can_move",can_move)
            if not can_move:
        ...
        if bool(chess_message['can_claim_draw']):
            print("claim_draw")
...
def start_play(id):
    ...
    if full_print:print(response.json()['status'])
